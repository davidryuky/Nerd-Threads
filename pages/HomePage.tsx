        ...
        <div className="space-y-20 md:space-y-32 pb-20">
            ...
            <section className="relative h-[70vh] md:h-[90vh] flex items-center justify-center text-center text-white overflow-hidden">
                ...
                <div className="relative z-20 p-4 animate-fade-in">
                    <h1 className="text-5xl md:text-8xl font-bold uppercase text-glow-gold tracking-widest">Forje Sua Lenda</h1>
            ...
            <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8">
                    {categories.map(category => (
                        <Link to="/catalog" key={category.name} className="relative rounded-lg overflow-hidden group aspect-w-4 aspect-h-3 border-2 border-transparent hover:border-amber-400/50 transition-all duration-300">
                            ...
                            <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex items-end justify-center p-4">
                                <h3 className="text-2xl font-bold text-white group-hover:text-glow-gold transition-colors">{category.name}</h3>
                            </div>
                        </Link>
                    ))}
                </div>
            ...
            {/* Offer Section */}
            <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                 <div className="bg-gray-800 rounded-lg p-8 md:p-12 flex flex-col md:flex-row items-center gap-8 border border-amber-500/30 shadow-lg shadow-amber-500/10">
                    <div className="md:w-1/2">
                        <img src={offerProduct.image} alt={offerProduct.name} className="rounded-lg w-full aspect-square object-cover border-2 border-amber-500/50"/>
                    </div>
                    <div className="md:w-1/2 text-center md:text-left">
                        <h3 className="text-md font-bold text-amber-400 uppercase tracking-widest">Tesouro em Destaque</h3>
                        ...
                        <div className="flex items-center justify-center md:justify-start gap-4 mt-6">
                             <p className="text-3xl font-bold text-glow-gold">R$ {offerProduct.price.toFixed(2)}</p>
                             <p className="text-xl text-gray-500 line-through">R$ 119,90</p>
                        </div>
                        <Link to={`/product/${offerProduct.id}`} className="mt-8 inline-block rpg-button">
